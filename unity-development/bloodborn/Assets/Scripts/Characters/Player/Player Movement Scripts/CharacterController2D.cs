...
public class CharacterController2D : ObjectController2D {
    ...
    private void SetAnimations() {
        ...
        brain.SetFloat(ANIMATION_H_SPEED, speed.x);
        brain.SetFloat(ANIMATION_V_SPEED, TotalSpeed.y);
        brain.SetFloat(ANIMATION_EX_SPEED, externalForce.x);
        brain.SetBool(ANIMATION_GROUNDED, collisions.onGround);
        brain.SetBool(ANIMATION_DASHING, Dashing);
        brain.SetBool(ANIMATION_WALL, collisions.hHit);
        brain.SetBool(ANIMATION_FACING, FacingRight);
        brain.SetBool(ANIMATION_LADDER, OnLadder);
        brain.SetBool(ANIMATION_INVULNERABLE, Invulnerable);
    }
